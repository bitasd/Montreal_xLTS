...
class Mixin:
    ...
    def node_winc_segm_wsam_name(self, tname: str = '', this_node_d: str = '') -> list:
        ...
        while len(self.nodes_reversed_dict[f'{this_node_d}'].incident_segments) < index_control:
            those_incid_seg_id_ = self.nodes_reversed_dict[f'{this_node_d}'].incident_segments[index_control]
                ...
                if len(end_node_those_inc_segs) > 0:
                    ...
                    for incsegid_endnode_those_segs in self.nodes_reversed_dict[
                        ...
                        if is_also_on_dual_cargway and it_has_samname:
                            ...
                            # only include nodes that are less than 25m far away from this_node
                            end_node_d = self.nodeid_to_reversedicindex[end_node_those_inc_seg]
                            end_node_coords = (self.nodes_reversed_dict[f'{end_node_d}'].lon,
                                               self.nodes_reversed_dict[f'{end_node_d}'].lat)

                            this_node_coords = (self.nodes_reversed_dict[f'{this_node_d}'].lon,
                                                self.nodes_reversed_dict[f'{this_node_d}'].lat)
                            if geod.geometry_length(LineString([Point(end_node_coords),
                                                                Point(this_node_coords)])) < 25:
                                is_short_dist = True
                            else:
                                is_short_dist = False
    ...
    # def relation_geometry(self, via_coords, fg, tg, is_multi=False):
    #     geod = pyproj.Geod(ellps='WGS84')
    #     # create two lines each starting from the intersection node pointing to the from/two's closes vertices
    #     # limit the length of each to ~25 m or smaller
    #
    #     if is_multi:
    #         line1 = LineString(
    #             [Point(tuple(list(map(float, via_coords)))), Point(tuple(list(map(float, fg))))])
    #         line2 = LineString(
    #             [Point(tuple(list(map(float, via_coords)))), Point(tuple(list(map(float, tg))))])
    #         line1_width = geod.geometry_length(line1)
    #         line2_width = geod.geometry_length(line2)
    #         if (line1_width <= 50) and (line2_width <= 50):
    #             rels_geom = LineString([Point(tuple(list(map(float, fg)))),
    #                                     Point(tuple(list(map(float, via_coords)))),
    #                                     Point(tuple(list(map(float, tg))))]
    #                                    )
    #         elif (line1_width > 50) and (line2_width <= 50):
    #             cp1 = self.split_line(line1, 0.00050)
    #             rels_geom = LineString([Point(cp1[0].coords[1]),
    #                                     Point(tuple(list(map(float, via_coords)))),
    #                                     Point(tuple(list(map(float, tg))))]
    #                                    )
    #         elif (line1_width <= 50) and (line2_width > 50):
    #             cp2 = self.split_line(line2, 0.00050)
    #             rels_geom = LineString([Point(tuple(list(map(float, fg)))),
    #                                     Point(tuple(list(map(float, via_coords)))),
    #                                     Point(cp2[0].coords[1])]
    #                                    )
    #         elif (line1_width > 50) and (line2_width > 50):
    #             cp1 = self.split_line(line1, 0.00050)
    #             cp2 = self.split_line(line2, 0.00050)
    ...
    #         line1_width = geod.geometry_length(line1)
    #         line2_width = geod.geometry_length(line2)
    #         if (line1_width <= 25) and (line2_width <= 25):
    #             rels_geom = LineString([Point(tuple(list(map(float, fg)))),
    #                                     Point(tuple(list(map(float, via_coords)))),
    #                                     Point(tuple(list(map(float, tg))))]
    #                                    )
    #         elif (line1_width > 25) and (line2_width <= 25):
    #             cp1 = self.split_line(line1, 0.00024)
    #             rels_geom = LineString([Point(cp1[0].coords[1]),
    #                                     Point(tuple(list(map(float, via_coords)))),
    #                                     Point(tuple(list(map(float, tg))))]
    #                                    )
    #         elif (line1_width <= 25) and (line2_width > 25):
    #             cp2 = self.split_line(line2, 0.00024)
    #             rels_geom = LineString([Point(tuple(list(map(float, fg)))),
    #                                     Point(tuple(list(map(float, via_coords)))),
    #                                     Point(cp2[0].coords[1])]
    #                                    )
