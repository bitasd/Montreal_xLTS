...
class GeoJuncs:
    ...
    def junc_layer(self, my_id: str = None):
        if my_id is None:
            my_id = 'ID_TRC'
        self.field_id = my_id
        pair_nodes = {}
        ...
        for el in list(self.nodes_reversed_dict.values()):
            ...
            if len(el.dual_name_to_nodeid) == 0 and len(el.incident_segments) > 2:
                ...
                for leg in legs_:
                    ...
                    duals.append(self.get_segment_attributes(leg, [list(self.field_val_spec_dual_cargwy.keys())[0]])[0])
                    road_names.append(self.get_segment_attributes(leg, [self.field_street_name])[0])
            ...
            # MULTI NODE INTERSECTIONS
                ...
                found_nodes = [pid for pid in el.dual_name_to_nodeid.values()]
                pair_nodes[el.id] = list(np.unique(np.array(found_nodes)))
        ...
        for jid in range(len(pairs_uniq)):
            ...
            for inc_segs_i in legs_:
                ...
                duals.append(self.get_segment_attributes(inc_segs_i, ['ONEWAY'])[0])
                ...
                # Bearing relative to the original node (not the averaged point) within the junction
